...
EFFECT_TUNE_MAGICIAN_X	  =247--调弦之魔术师（超量）
...
EFFECT_MUST_USE_MZONE			=265 --可以在通常怪兽区特殊召唤额外卡组的怪兽
EFFECT_BECOME_LINKED_ZONE	=266 --只能通常召唤/覆盖或特殊召唤到指定区域 (GUY热舞)
...
EFFECT_TUNER_MATERIAL_LIMIT  =353   --同调素材种类限制
EFFECT_CANNOT_LOSE_DECK   =400   --不会因无法抽卡败北
EFFECT_CANNOT_LOSE_LP		=401 --不会因LP归0败北
EFFECT_CANNOT_LOSE_EFFECT	=402 --不会因效果败北
EFFECT_BP_FIRST_TURN			=403	--先攻可以进入战斗阶段
EFFECT_UNSTOPPABLE_ATTACK	=404 --攻击无法阻止（不受EFFECT_CANNOT_ATTACK + EFFECT_CANNOT_ATTACK_ANNOUNCE + EFFECT_CANNOT_DIRECT_ATTACK影响）
EFFECT_ALLOW_NEGATIVE		=405 --负星级
EFFECT_SELF_ATTACK		=406  --玩家可以攻击自己的怪兽（PLAYER TARGET）
EFFECT_BECOME_QUICK	=407   --魔法速攻化（会得到速攻种类）
EFFECT_LEVEL_RANK			=408 --增加阶级
EFFECT_RANK_LEVEL			=409 --增加星级
EFFECT_LEVEL_RANK_S	=410   --增加阶级（与星级同步）
EFFECT_RANK_LEVEL_S	=411   --增加星级（与阶级同步）
EFFECT_UPDATE_LINK		=420  --改变连接数（连接数增加/减少）
EFFECT_CHANGE_LINK		=421  --设置连接数（永续型效果、连接数变成X特殊召唤）
EFFECT_CHANGE_LINK_FINAL		=422	--设置连接数（所有入连锁的改变连接数）
EFFECT_ADD_LINKMARKER		=423 --增加连接箭头
EFFECT_REMOVE_LINKMARKER		=424	--删除连接箭头
EFFECT_CHANGE_LINKMARKER		=425	--改变连接箭头
